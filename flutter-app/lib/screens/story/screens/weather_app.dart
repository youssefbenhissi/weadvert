...
class WeatherApp extends StatefulWidget {
  ...
  @override
  _WeatherAppState createState() => _WeatherAppState(currentPage: this.index);
}

class _WeatherAppState extends State<WeatherApp> {
  int currentPage;
  String bgImg;
  _WeatherAppState({this.currentPage});
  ...
  Widget build(BuildContext context) {
    print("waaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" + currentPage.toString());
              ...
              child: Row(
                children: [
                  for (int i = 0; i < locationList.length; i++)
                    if (i == currentPage) SliderDot(true) else SliderDot(false)
                ],
              ),
            ),
            TransformerPageView(
